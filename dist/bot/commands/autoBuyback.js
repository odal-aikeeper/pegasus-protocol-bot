...
exports.autoBuybackCommand = autoBuybackCommand;
const db_1 = __importDefault(require("../../database/db"));
async function autoBuybackCommand(ctx) {
    const telegramId = ctx.from.id;
    const args = ctx.message?.text?.split(' ').slice(1);
    if (!args || args.length < 2) {
        await ctx.reply(`🎯 *Set Price Trigger*\n\n` +
            `Usage: /auto_buyback <trigger_price> <amount>\n\n` +
            `Example: \`/auto_buyback 0.05 2\`\n\n` +
            `This will automatically buy when price reaches or drops below the trigger price.`, { parse_mode: 'Markdown' });
        return;
    }
    ...
    try {
        db_1.default.prepare('INSERT INTO price_triggers (telegram_id, trigger_price, amount_sol, is_active) VALUES (?, ?, ?, 1)').run(telegramId, triggerPrice, amount);
        const currentPrice = 0.075;
        await ctx.reply(`🎯 *Price Trigger Set!*
...
Cancel anytime with /cancel_trigger`, { parse_mode: 'Markdown' });
    }
    catch (error) {
        console.error('Auto buyback error:', error);
        await ctx.reply(`❌ Failed to set price trigger.\n\n` +
            `Please try again later.`);
    }
}
//# sourceMappingURL=autoBuyback.js.map