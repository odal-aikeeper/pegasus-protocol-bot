...
exports.updateMarketCap = updateMarketCap;
exports.formatMarketCap = formatMarketCap;
exports.createProgressBar = createProgressBar;
exports.startMarketCapTracking = startMarketCapTracking;
...
async function updateMarketCap() {
    try {
        ...
        // Update database
        db_1.default.prepare('UPDATE lottery_pool SET current_market_cap = ?, last_updated = strftime("%s", "now") WHERE id = 1').run(marketCap);
...
async function checkMilestoneReached() {
    const lottery = db_1.default.prepare('SELECT * FROM lottery_pool WHERE id = 1').get();
    if (!lottery)
        return;
    ...
    if (currentMC >= milestone) {
        ...
        db_1.default.prepare('UPDATE lottery_pool SET next_milestone_market_cap = ? WHERE id = 1').run(nextMilestone);
 ...
 */
function startMarketCapTracking() {
    const tokenMint = process.env.TOKEN_MINT_ADDRESS;
    if (!tokenMint || tokenMint === 'your_token_mint_address_here') {
...
//# sourceMappingURL=marketCapTracker.js.map